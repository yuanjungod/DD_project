...
def _attach_run_children(db: Session, engagement_id: str, run: AgentRun, result: dict) -> None:
...
def finalize_agent_run(db: Session, engagement_id: str, run_id: str, result: dict) -> AgentRun:
...
def upsert_incremental_run_progress(
    ...
    *,
    ...
    if run is None or run.engagement_id != engagement_id:
        raise ValueError("Run not found or engagement mismatch")
